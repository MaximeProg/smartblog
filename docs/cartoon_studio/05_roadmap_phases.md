...
**Livrables :**
- Series Memory (RAG-backed) et modes de continuité ; content calendar ; planification quotidienne et avancée (timezones, heures calmes, jours fériés, fenêtres de campagne) ; workflow automatique en 18 étapes avec checkpointing ; génération de sujet avec évitement de répétition ; workflow de brouillon et notifications ; publication automatique contrôlée avec défauts de catégorie sensible ; modes de publication ; mesure d'usage, budgets de coût et alertes ; usage/billing dashboard ; console de modération admin (basique) ; paramètres de publication ; pipeline de fact-checking pour les projets factuels ; écrans UI 14–16, 18, 20 (basique).

**Gate** : un projet configuré pour la génération quotidienne produit des brouillons revuables pendant **7 exécutions programmées consécutives sans répétition de sujet**, avec les coûts enregistrés et les propriétaires notifiés.
